...
func TestAttributeBuilder(t *testing.T) {
	...
	if *requestModel.Amount != float64(amount) {
		t.Errorf("amount convertion to DynamoDB attribute not correct, got = %v, want = %v", *requestModel.Amount, amount)
		return
	}

	if *requestModel.CategoryId != category {
		t.Errorf("category convertion to DynamoDB attribute not correct, got = %v, want = %v", *requestModel.CategoryId, category)
		return
	}
	
	if *requestModel.Description != description {
		t.Errorf("description convertion to DynamoDB attribute not correct, got = %v, want = %v", *requestModel.Description, description)
		return
	}
}
