...
func TestAttributeBuilder(t *testing.T) {
	...
	body := "{\"pk\":\"" + pk + "\",\"creation_date\":\"" + creationDate + "\",\"category_type\":\"" + categoryType + "\",\"category_name\":\"" + categoryName + "\",\"updated_date\":\"" + updatedDate + "\"}"
	...
	if *(*got["category_type"]).S != categoryType {
		t.Errorf("name category_type to DynamoDB attribute not correct, got = %v, want = %v", *(*got["category_type"]).S, categoryType)
		return
	}

	if *(*got["category_name"]).S != categoryName {
		t.Errorf("name category_name to DynamoDB attribute not correct, got = %v, want = %v", *(*got["category_name"]).S, categoryName)
		return
	}
}
