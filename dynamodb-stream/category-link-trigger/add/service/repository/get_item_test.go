...
func TestGetCategoryRuleItem(t *testing.T) {
	thisTime := time.Now()
	...
	item, err := GetCategoryRuleItem(mockSvc, &pk, &sk)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetCategoryItem(t *testing.T) {
	thisTime := time.Now()
	nowString := thisTime.Format("2006-01-02 15:04:05 Monday")
	t.Log("Starting unit test at " + nowString)
	...
	item, err := GetCategoryItem(mockSvc, &pk, &sk)
	if err != nil {
		t.Fatal(err)
	}
