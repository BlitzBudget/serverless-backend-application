...
type InvestmentQueryParameter struct {
	...
	InvestedAmount *float64 `validate:"required" json:":r"`
	UpdatedDate    *string  `validate:"required" json:":u"`
}

// convert item to dynamodb attribute
func InvestmentParseToQueryParameter(currentValue *float64, investedAmount *float64) map[string]*dynamodb.AttributeValue {
